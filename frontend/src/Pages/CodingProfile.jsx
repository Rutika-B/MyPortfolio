    ...
    const getProfile = async () => {
      ...
      if (!leetcodeData || Date.now() - leetcodeTimestamp > 300000) {
        const response = await axios.get(
          "https://rutika-coding-profile.vercel.app/api/routes/leetcode"
        );
      ...
      if (!codeforcesData || Date.now() - codeforcesTimestamp > 300000) {
        const response2 = await axios.get(
          "https://rutika-coding-profile.vercel.app/api/routes/leetcode/codeforces"
        );
  ...
  useEffect(() => {
    ...
    if (profile.data) {
      ...
      for (let i = 0; i < profile.data.userContestRankingHistory.length; i++) {
        if (
          profile.data.userContestRankingHistory[i].contest.startTime ===
          1632623400
        )
          flag = true;
        ...
        if (flag === true) {
          let date = new Date(
            profile.data.userContestRankingHistory[i].contest.startTime * 1000
          );
          ...
          yaxis.push(newDate);
          contestNameArr.push(
            profile.data.userContestRankingHistory[i].contest.title
          );
  ...
  useEffect(() => {
    if (profile2.length > 0) {
      ...
      for (let i = 0; i < profile2[0].result.length; i++) {
        let date = new Date(
          profile2[0].result[i].ratingUpdateTimeSeconds * 1000
        );
        let newDate = monthName[date.getMonth()] + "," + date.getFullYear();
    ...
    >
      <div className="max-w-screen-lg p-4 mx-auto flex flex-col justify-center w-full h-full">
        ...
        <div className="main_container_coding">
          {profile.data && profile2.length > 0 ? (
            <div className="main_container_coding_inner">
              <div className="leetcode_container">
                ...
                <div className="leetcode_description">
                  <div className="left_column_leetcode">
                    <div className="leetcode_stats_label">
                      Current Rating :{" "}
                      <div className="leetcode_stats_text">
                        {" "}
                        {profile.data &&
                          profile.data.userContestRanking.rating.toFixed(2)}
                      </div>
                    </div>
                    <div className="leetcode_stats_label">
                      Number of attended contests :{" "}
                      <div className="leetcode_stats_text">
                        {" "}
                        {profile.data &&
                          profile.data.userContestRanking
                    ...
                    <div className="leetcode_stats_label">
                      ...
                      <div className="leetcode_stats_text">
                        {profile.data &&
                          profile.data.userContestRanking.globalRanking}{" "}
                      </div>
                    </div>
                    <div className="leetcode_stats_label">
                      I am among the top{" "}
                      <div className="leetcode_stats_text">
                        {" "}
                        {profile.data &&
                          profile.data.userContestRanking.topPercentage}
                        %{" "}
                      </div>{" "}
                      users
                    </div>
                  ...
                  <div className="right_column_leetcode">
                    <div className="leetcode_stats_label">
                      ...
                      <div className="leetcode_stats_text">
                        {profile.data &&
                          profile.data.matchedUser.submitStats
                            .acSubmissionNum[0].count}
                      </div>{" "}
                    </div>
                    <div className="easy_color">
                      Easy :{" "}
                      <div className="leetcode_stats_text">
                        {profile.data &&
                          profile.data.matchedUser.submitStats
                  ...
                  <div className="right_column_codeforces">
                    <div className="leetcode_stats_label">
                      ...
                      <div className="leetcode_stats_text">
                        ...
                        {profile2.length > 0 ? (
                          formatDate(
                            profile2[1].result[0].lastOnlineTimeSeconds
                          )
                        ) : (
                          <></>
                        )}
                    ...
                    <div className="leetcode_stats_label">
                      ...
                      <div className="leetcode_stats_text">
                        ...
                        {profile2.length > 0 ? (
                          formatDate(
                            profile2[1].result[0].registrationTimeSeconds
                          )
                        ) : (
                          <></>
                        )}
                    ...
                    <div className="leetcode_stats_label">
                      ...
                      <div className="leetcode_stats_text">
                        {" "}
                        {profile2.length > 0 ? (
                          profile2[1].result[0].contribution
                        ) : (
                          <></>
                        )}
                      </div>
                    ...
                    <div className="leetcode_stats_label">
                      ...
                      <div className="leetcode_stats_text">
                        {" "}
                        {profile2.length > 0 ? (
                          profile2[1].result[0].friendOfCount
                        ) : (
                          <></>
                        )}
                      </div>
